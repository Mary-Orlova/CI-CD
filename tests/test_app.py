...
async def test_read_recipe_by_id(client):
    ...
    recipe_data = {"title": "Test Recipe", "description": "Test Description", "cook_time": 30, "ingredients": []}  # Include ingredients
...
async def test_update_recipe(client):
    # Create a recipe first
    recipe_data = {"title": "Old Recipe", "description": "Old Description", "cook_time": 60, "ingredients": []}  # Include ingredients
    ...
    # Update the recipe
    update_data = {"title": "Updated Recipe", "description": "Updated Description", "cook_time": 20, "ingredients": []}  # Include ingredients
...
async def test_delete_recipe(client):
    # Create a recipe first
    recipe_data = {"title": "Recipe to Delete", "description": "Description to Delete", "cook_time": 30, "ingredients": []}  # Include ingredients
    create_response = await client.post("/recipes/", json=recipe_data)
    assert create_response.status_code == 200, f"Expected 200, but got {create_response.status_code} with content: {create_response.text}"
    created_recipe = create_response.json()
    recipe_id = created_recipe["id"]
